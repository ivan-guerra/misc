...
PlayerShip::PlayerShip(IDiceInvaders* engine, ISprite* ship_sprite, ISprite* rocket_sprite,
        const std::pair<float, float>& res) :
...
void PlayerShip::update()
{
    ...
    ship_sprite_->draw((int)position_.first, (int)position_.second);
...
void PlayerShip::kb_event_handler()
{
    ...
    float move = (curr_time - last_update_time_) * 360.0f;
