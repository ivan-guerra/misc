...
public:
    ...
    AlienList(IDiceInvaders* engine, ISprite* alien_sprite, ISPrite* bomb_sprite, const std::pair<uint32_t,uint32_t>& screen_res);
     ...
     */
    void resolve_collisions(PlayerShip* player, int direction);
...
private:
    static const uint32_t TOTAL_ALIENS = 30; /*!< The maximum number of aliens that can be in game simultaneously. */
    static const uint32_t ALIENS_PER_ROW = 10; /*!< The number of aliens that appear per row. */

    /*!
     * \brief Determine whether there is a collision between \p pos1 and \p pos2.
     * \return True if \p pos1 and \p pos2 collide.
     */
    bool has_collision(const std::pair<uint32_t,uint32_t>& pos1, const std::pair<uint32_t,uint32_t>& pos2) const;
